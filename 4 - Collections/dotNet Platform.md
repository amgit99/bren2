...
The Projects have a type, it could be a <span style="color:#e1db3d">class library</span> (collection of functionality, for shared purposes, does not have a main function), or an console app (one with UI that consumes the functionality in other libraries). A project may contain common functionality that can be used by other projects, for this we must add a reference of the library project in the project that intends to use the library functionality.
